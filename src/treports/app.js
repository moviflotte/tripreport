...
import { buildVehicleReportXlsx, XLSX_CONTENT_TYPE } from "./xlsx-builder.js";
...
const excelButton = document.querySelector("#excelReport");
...
function renderRows(rows) {
  ...
  excelButton.disabled = rows.length === 0;
...
async function downloadExcel() {
  ...
  const selectedDate = dateInput.value;
  excelButton.classList.add("is-loading");

  try {
    const buffer = await buildVehicleReportXlsx(currentRows, selectedDate);
    const blob = new Blob([buffer], { type: XLSX_CONTENT_TYPE });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");

    anchor.href = url;
    anchor.download = `rapport-vehicules-${selectedDate}.xlsx`;
    anchor.click();
    URL.revokeObjectURL(url);
  } catch (error) {
    console.error("[treports] failed to build Excel report", error);
  } finally {
    excelButton.classList.remove("is-loading");
  }
}
